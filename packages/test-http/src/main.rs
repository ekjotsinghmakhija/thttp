...
use test_http::report::{curl, html, json, junit, tap};
use test_http::runner;
use test_http::runner::THttpResult;
use test_http::util::redacted::Redact;
use test_http_core::input::Input;
use test_http_core::text;
...
/// Structure that stores the result of an THttp file execution, and the content of the file.
#[derive(Clone, Debug, PartialEq, Eq)]
struct THttpRun {
    /// Source string for this [`THttpFile`]
    content: String,
    /// Content's source file
    filename: Input,
    test_http_result: THttpResult,
}

/// Executes THttp entry point.
fn main() {
    ...
    // We'll use a more advanced logger for rich error report when running THttp files.
    ...
    let runs = match runs {
        ...
        // done in the execution of the THttp files, inside the crates (and not in the main).
...
fn export_results(
    ...
    let secrets = runs
        .iter()
        .flat_map(|r| r.test_http_result.variables.secrets())
        .collect::<HashSet<_>>();
...
fn create_curl_export(runs: &[THttpRun], filename: &Path, secrets: &[&str]) -> Result<(), CliError> {
    let results = runs.iter().map(|r| &r.test_http_result).collect::<Vec<_>>();
...
fn create_junit_report(
    ...
    let testcases = runs
        .iter()
        .map(|r| junit::Testcase::from(&r.test_http_result, &r.content, &r.filename))
        .collect::<Vec<_>>();
...
fn create_tap_report(runs: &[THttpRun], filename: &Path) -> Result<(), CliError> {
    let testcases = runs
        .iter()
        .map(|r| tap::Testcase::from(&r.test_http_result, &r.filename))
        .collect::<Vec<_>>();
...
fn create_html_report(runs: &[THttpRun], dir_path: &Path, secrets: &[&str]) -> Result<(), CliError> {
    ...
    for run in runs.iter() {
        let result = &run.test_http_result;
...
fn create_json_report(runs: &[THttpRun], dir_path: &Path, secrets: &[&str]) -> Result<(), CliError> {
    // We ensure that the containing folder exists.
    let store_path = dir_path.join("store");
    std::fs::create_dir_all(&store_path)?;
    ...
    let testcases = runs
        .iter()
        .map(|r| json::Testcase::new(&r.test_http_result, &r.content, &r.filename))
        .collect::<Vec<_>>();
...
/// Returns an exit code for a list of THttpResult.
fn exit_code(runs: &[THttpRun]) -> i32 {
    let mut count_errors_runner = 0;
    let mut count_errors_assert = 0;
    for run in runs.iter() {
        let errors = run.test_http_result.errors();
        if errors.is_empty() {
...
fn create_cookies_file(
    ...
    if let Err(err) = test_http::util::path::create_dir_all(filename) {
...
# This file was generated by THttp
...
"#
    ...
    for run in runs.iter() {
        ...
        s.push('\n');
        for cookie in run.test_http_result.cookies.iter() {
