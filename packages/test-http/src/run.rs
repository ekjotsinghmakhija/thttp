...
use test_http::parallel::job::{Job, JobResult};
use test_http::parallel::runner::ParallelRunner;
use test_http::pretty::PrettyMode;
use test_http::runner::{THttpResult, Output, VariableSet};
use test_http::util::term::{Stdout, WriteMode};
use test_http::{output, parallel, runner};
use test_http_core::error::{DisplaySourceError, OutputFormat};
use test_http_core::input::Input;
use test_http_core::types::Count;

use crate::cli::options::CliOptions;
use crate::cli::CliError;
use crate::{cli, THttpRun};

/// Runs THttp `files` sequentially, given a current directory and command-line options (see
/// [`crate::cli::options::CliOptions`]). This function returns a list of [`THttpRun`] results or
/// an error.
pub fn run_seq(
    files: &[Input],
    current_dir: &Path,
    options: &CliOptions,
) -> Result<Vec<THttpRun>, CliError> {
    let mut runs = vec![];
    ...
    for filename in queue {
        ...
        // Run our THttp file now, we can only fail if there is a parsing error.
        // The parsing error is displayed in the `execute` call, that's why we gobble the error
        // string.
        let Ok(test_http_result) = runner::run(
            &content,
            Some(&filename),
            &runner_options,
            &variables,
        ...
        // representation of the full THttp result.
...
/// Prints a `test_http_result` either as a raw HTTP response (last body of the run), or in a structured
/// JSON way.
///
...
fn print_output(
    ...
    let output_last_body = test_http_result.success
        && !options.interactive
...
/// Runs THttp `files` in parallel, given a current directory and command-line options (see
/// [`crate::cli::options::CliOptions`]). This function returns a list of [`THttpRun`] results or
/// an error.
pub fn run_par(
    files: &[Input],
    ...
    let results = results.into_iter().map(THttpRun::from).collect();
    Ok(results)
}

impl From<JobResult> for THttpRun {
    fn from(job_result: JobResult) -> Self {
        THttpRun {
            content: job_result.content,
            filename: job_result.job.filename,
            test_http_result: job_result.test_http_result,
        }
    }
}
